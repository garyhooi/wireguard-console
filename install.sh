...
if [[ "${UPGRADE}" == false ]]; then
  ...
  if [[ -z "${CONSOLE_DOMAIN:-}" && -r /dev/tty ]]; then
    read -rp "Domain or public IP this console will be reachable at (e.g. vpn.yourcompany.com, or 203.0.113.5): " CONSOLE_DOMAIN < /dev/tty
  fi
...
# Empty for domain mode (public ACME certificates); "tls internal" for IP-only mode.
