...
# usage: stamp_app_version <file> [resolved-value] — set (or append) the
# APP_VERSION line in a .env file. Runs on EVERY install/update so the
# running API always reports the version it was just updated to (the console's
# update check compares this against the newest GitHub release). Resolution
# order: the resolved value passed by the caller (a caller-exported APP_VERSION
# snapshotted before any `source .env` clobbers it), else this checkout's
# repo-root VERSION file, else "dev". The value is a plain "1.2.3" — no
# quoting issues.
stamp_app_version() {
  ...
  if grep -q '^APP_VERSION=' "$file"; then
    APP_VERSION_VAL="${value}" awk '
      /^APP_VERSION=/ { print "APP_VERSION=" ENVIRON["APP_VERSION_VAL"]; next }
      { print }
    ' "$file" > "$file.tmp" && mv "$file.tmp" "$file"
  else
    printf "\nAPP_VERSION=%s\n" "${value}" >> "$file"
...
# APP_VERSION (the release this checkout represents) is stamped into .env by
# stamp_app_version() below on EVERY install/update run — the running API
# reads it to show the current console version and compare against the newest
# GitHub release. Derivation happens at stamp time (after .env handling) from
# the repo-root VERSION file, with a caller-exported APP_VERSION taking
# priority — see stamp_app_version(). Snapshot the caller's export now, before
# any `source .env` below could clobber it with a stale stored value.
CALLER_APP_VERSION="${APP_VERSION:-}"
...
# APP_VERSION is the release this checkout was deployed from (repo-root
# VERSION file); the console shows it in the top bar and compares it against
...
# Stamp the resolved APP_VERSION into .env (fresh heredoc and kept .env
...
# Version line for the summary — read back from .env (the stamped source of
# truth), like DOMAIN_SHOWN/TLS_MODE_SHOWN above. The `|| true` guards keep an
# absent key from aborting the script under `set -euo pipefail`.
APP_VERSION_SHOWN="$(grep '^APP_VERSION=' .env | cut -d= -f2- || true)"
APP_VERSION_SHOWN="${APP_VERSION_SHOWN:-${APP_VERSION:-dev}}"
...
${FIRST_ADMIN_BLOCK}
 ...
 The console shows this version in its top bar and flags newer GitHub
 releases there — updates always run this same installer, and back up on
 the Backups page first.
==================================================================
SUMMARY
